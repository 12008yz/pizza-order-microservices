...
import type { Provider, ApiResponse } from '@tariff/shared-types';
...
class ProvidersService {
  ...
  async getProvidersByAddress(params: GetProvidersByAddressParams): Promise<ApiResponse<Provider[]>> {
    const queryParams = new URLSearchParams();
    ...
    if (params.streetId) queryParams.append('streetId', params.streetId.toString());
    if (params.city) queryParams.append('city', params.city);
    if (params.street) queryParams.append('street', params.street);
    if (params.house) queryParams.append('house', params.house.toString());
    return apiClient.get<Provider[]>(`/api/providers/by-address?${queryParams.toString()}`);
  }
}
